...
public class StudentServiceTests
{
    ...
    public void GetStudentsWithMinAverageGrade_ReturnsCorrectStudents()
    {
        ...
        Assert.Equal(2, result.Count);
        Assert.True(result.All(student => student.Grades.Average() >= 4.2));
    }
    ...
    public void GetStudentsOrderedByName_ReturnsCorrectOrder()
    {
        ...
        Assert.Equal(_testStudents.OrderBy(student => student.Name), result);
    }

    [Fact]
    public void GroupStudentsByFaculty_ReturnsCorrectOrder()
    {
        var result = _service.GroupStudentsByFaculty();
        Assert.Equal(2, result.Count);
        Assert.Contains("ФИТ", result.Select(group => group.Key));
        Assert.Contains("Экономика", result.Select(group => group.Key));
    }
